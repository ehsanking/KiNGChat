#!/bin/bash
# ╔══════════════════════════════════════════════════════════════════╗
# ║          KiNGChat Professional Installer v2.6                   ║
...
print_header() {
    ...
    echo -e "${GOLD}================================================================${NC}"
    echo -e "${WHITE}  Installer v2.6 — DNS Control | Auto-SSL | Smart Skip         ${NC}"
    echo -e "${GOLD}================================================================${NC}"
    echo ""
}
...
OBJECT_STORAGE_ROOT=object_storage
# Optional external object storage (disabled by default)
...
# Generated by KiNGChat Installer v2.6
...
launch_services() {
    ...
    if compose_service_exists "minio"; then
        ...
        STORAGE_BACKEND_SUMMARY="Local filesystem (no MinIO container)"
        log_info "Compose file does not define a MinIO service. Using local filesystem storage."
    fi
...
main() {
    ...
    log_info "Initializing KiNGChat Professional Installer v2.6..."
